...
- To override some configuration options, you can use the flag ``--set``, for
  ...
  documents, you can just type
    ...
    .. code:: shell
        ...
        dldawn --set editor gedit --set opentool firefox edit
        dldawn --set editor gedit --set opentool firefox open

- If you want to list the libraries and pick one before sending a database
  query to dldawn, use ``--pick-lib`` as such

    .. code:: shell

        dldawn --pick-lib open 'einstein relativity'

Cli
^^^
.. click:: dldawn.commands.default:run
    :prog: dldawn
    :commands: []
...
class MultiCommand(click.MultiCommand):
    ...
    scripts = dldawn.commands.get_scripts()
    scripts.update(dldawn.commands.get_external_scripts())
    ...
    def get_command(self, ctx, name):
        ...
        # from dldawn.commands.external import external_cli as cli
        # from dldawn.commands.external import get_command_help
...
@click.version_option(version=dldawn.__version__)
@click.option(
    "-v",
    "--verbose",
