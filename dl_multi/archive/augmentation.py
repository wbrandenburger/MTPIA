...
def rnd_crop_flip(image, annotation, size, minscale, maxscale):
  ...
  crop = tf.image.random_crop(both, crop_size)
...
#   function ----------------------------------------------------------------
# ---------------------------------------------------------------------------
def blubbblubb(image, height, size, minscale, maxscale):
...
def full_img(image, annotation, size, minscale, maxscale):
  ...
  both = tf.concat([image, annotation], 2)
  both = tf.image.resize_with_crop_or_pad(both, size[0], size[1])
  ...
  img_res = tf.image.resize(img_crop, tf.stack([y, x]), method=tf.image.ResizeMethod.BILINEAR)
  anno_res = tf.image.resize(anno_crop, tf.stack([y, x]), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)

  img_crop = tf.image.resize_with_crop_or_pad(img_res, size[0], size[1])
  anno_crop = tf.image.resize_with_crop_or_pad(anno_res, size[0], size[1])
